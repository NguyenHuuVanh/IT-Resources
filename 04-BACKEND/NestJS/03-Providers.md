...
## 5. Các loại Provider (Custom Providers) — Chi tiết cho phỏng vấn
...
- **`provide` (token)**: Có thể là **class**, **abstract class**, **string**, hoặc **Symbol**.
- **`use___`**: Quyết định NestJS sẽ **tạo ra cái gì** và **bằng cách nào**.

NestJS hỗ trợ đúng **4 cách** định nghĩa custom provider. Dưới đây là phần **khái niệm chi tiết**, được viết dành riêng để bạn có thể trả lời tốt trong phỏng vấn:

### 5.1. `useClass` — Cung cấp instance từ một Class

**Khái niệm chi tiết (dành cho phỏng vấn):**

`useClass` là cách định nghĩa provider **mặc định** trong NestJS. Khi bạn khai báo một class vào mảng `providers`, NestJS sẽ hiểu ngầm rằng: **"Bất cứ khi nào ai đó yêu cầu token này, hãy tạo một instance mới của class được chỉ định bằng cách gọi `new ClassName()` và tự động quản lý vòng đời của nó."**

Về bản chất, `useClass` cho phép bạn **tách biệt giữa token (định danh) và implementation (lớp thực thi)**. Đây là điểm then chốt:

- Bạn có thể thay đổi implementation mà **không cần sửa code** ở nơi inject.
- Hỗ trợ tốt nguyên tắc **Dependency Inversion** (chữ D trong SOLID) khi kết hợp với `abstract class`.
- NestJS sẽ tự động thực hiện constructor injection cho class được chỉ định.

**Đặc điểm quan trọng cần nhớ khi phỏng vấn:**
- Mỗi lần token được yêu cầu → NestJS tạo **instance mới** (trừ khi scope là `DEFAULT`).
- Phù hợp khi bạn muốn kiểm soát **class nào** sẽ được khởi tạo cho một token.
- Thường dùng với `abstract class` làm token để ép các implementation phải tuân theo cùng contract.

**Câu trả lời mẫu khi phỏng vấn:**
> "`useClass` cho phép map một token sang một class cụ thể. NestJS sẽ tự động khởi tạo class đó và inject các dependency bên trong. Đây là cách linh hoạt để thay đổi implementation theo môi trường hoặc khi testing, đồng thời hỗ trợ Dependency Inversion Principle."
...
@Injectable() class DevConfigService extends ConfigService { get(key) { return `[DEV] ${key}`; } }
@Injectable() class ProdConfigService extends ConfigService { get(key) { return `[PROD] ${key}`; } }
...
`useValue` là cách cung cấp một **giá trị tĩnh** (object, primitive, hoặc instance đã được tạo sẵn) mà **không qua quá trình khởi tạo class** bởi NestJS Container. Khi dùng `useValue`, bạn đang nói với NestJS: **"Đừng gọi `new`, đừng chạy logic nào. Chỉ cần trả về đúng giá trị này mỗi khi token được yêu cầu."**
...
- Thường dùng khi giá trị đã được chuẩn bị sẵn từ bên ngoài (config, mock, third-party instance).

**Đặc điểm quan trọng cần nhớ khi phỏng vấn:**
- Token thường là **string** hoặc **Symbol** (không phải class).
- Khi inject phải dùng `@Inject('TOKEN')`.
- Rất mạnh trong **unit testing** để thay thế dependency thật bằng mock object.

**Câu trả lời mẫu khi phỏng vấn:**
> "`useValue` dùng khi tôi muốn inject một giá trị tĩnh hoặc object đã được khởi tạo sẵn. NestJS sẽ không tạo instance mới mà chỉ trả về đúng giá trị tôi cung cấp. Tôi thường dùng nó để cung cấp config hoặc mock dependency trong test."

**Ví dụ hay dùng:**
```ts
{ provide: 'APP_CONFIG', useValue: { apiKey: 'abc123', timeout: 5000 } }

// Mock trong test
{ provide: CatsService, useValue: { findAll: () => [{ id: 1, name: 'Mock' }] } }
```
...
`useFactory` là cách **linh hoạt và mạnh mẽ nhất** trong 4 loại. Thay vì chỉ định class hoặc giá trị tĩnh, bạn cung cấp một **hàm factory**. NestJS sẽ **gọi hàm này** và dùng **giá trị trả về** của hàm làm provider.

Điểm then chốt:
- Hàm có thể chứa **bất kỳ logic nào** (sync hoặc async).
- Bạn có thể **inject các provider khác** vào hàm thông qua mảng `inject`.
- NestJS sẽ **chờ Promise resolve** nếu factory trả về Promise.
...
**Đặc điểm quan trọng cần nhớ khi phỏng vấn:**
- Mảng `inject` quyết định những gì được truyền vào hàm (theo đúng thứ tự).
...
**Câu trả lời mẫu khi phỏng vấn:**
> "`useFactory` cho phép tôi tạo provider một cách linh hoạt bằng cách cung cấp một hàm. Hàm này có thể nhận các dependency khác qua `inject`, hỗ trợ async, và tôi có thể viết logic phức tạp để quyết định instance nào được tạo. Tôi thường dùng nó để tạo database connection dựa trên config động."
...
- Tiết kiệm tài nguyên vì tái sử dụng instance.
- Rất hữu ích khi refactor hoặc muốn hỗ trợ nhiều module.
...
### 5.5. Bảng so sánh nhanh 4 loại Custom Provider (dành cho phỏng vấn)
